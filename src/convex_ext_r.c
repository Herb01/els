...
static struct {
  ...
  els_convex_ext_r_state_t prev_state;
  els_spindle_direction_t prev_dir;

  // operation state
  els_convex_ext_r_op_state_t op_state;
  els_convex_ext_r_op_state_t prev_op_state;
...
static void els_convex_ext_r_keypad_process(void);
...
void els_convex_ext_r_start(void) {
  ...
  els_convex_ext_r.prev_op_state = ELS_CONVEX_EXT_OP_NA;
...
bool els_convex_ext_r_busy(void) {
  return els_convex_ext_r.state != ELS_CONVEX_EXT_IDLE;
}
...
void els_convex_ext_r_update(void) {
  ...
  if (els_convex_ext_r.locked != kp_locked) {
    els_convex_ext_r.locked = kp_locked;
    els_convex_ext_r_display_header();
  }
  ...
  switch (els_convex_ext_r.state) {
    ...
    case ELS_CONVEX_EXT_ACTIVE:
      els_convex_ext_r_run();
      break;
    case ELS_CONVEX_EXT_SET_FEED:
      els_convex_ext_r_set_feed();
      break;
    ...
    default:
      if (els_convex_ext_r.state & (ELS_CONVEX_EXT_SET_ZAXES | ELS_CONVEX_EXT_ZJOG))
        els_convex_ext_r_set_zaxes();
      else if (els_convex_ext_r.state & (ELS_CONVEX_EXT_SET_XAXES | ELS_CONVEX_EXT_XJOG))
        els_convex_ext_r_set_xaxes();
      break;
  }
...
static void els_convex_ext_r_keypad_process(void) {
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
      if (els_convex_ext_r.state == ELS_CONVEX_EXT_IDLE) {
        if (els_convex_ext_r.length > 0 && els_convex_ext_r.depth > 0) {
          els_convex_ext_r.state = ELS_CONVEX_EXT_PAUSED;
      ...
      break;
    case ELS_KEY_EXIT:
      if (els_convex_ext_r.state & (ELS_CONVEX_EXT_PAUSED | ELS_CONVEX_EXT_ACTIVE)) {
        els_convex_ext_r.state = ELS_CONVEX_EXT_IDLE;
        els_convex_ext_r.op_state = ELS_CONVEX_EXT_OP_IDLE;
      }
      break;
    case ELS_KEY_SET_FEED:
      if (els_convex_ext_r.state & (ELS_CONVEX_EXT_IDLE | ELS_CONVEX_EXT_PAUSED)) {
        els_convex_ext_r.state = ELS_CONVEX_EXT_SET_FEED;
...
static void els_convex_ext_r_run(void) {
  switch (els_spindle_get_direction()) {
    case ELS_S_DIRECTION_CW:
    case ELS_S_DIRECTION_CCW:
      if (els_convex_ext_r.state == ELS_CONVEX_EXT_PAUSED && els_spindle_get_counter() == 0)
        els_convex_ext_r.state = ELS_CONVEX_EXT_ACTIVE;
      break;
    default:
      if (els_convex_ext_r.state == ELS_CONVEX_EXT_ACTIVE)
        els_convex_ext_r.state = ELS_CONVEX_EXT_PAUSED;
      break;
  }
...
static void els_convex_ext_r_turn(void) {
  ...
  switch (els_convex_ext_r.op_state) {
    case ELS_CONVEX_EXT_OP_NA:
    case ELS_CONVEX_EXT_OP_IDLE:
      break;
    case ELS_CONVEX_EXT_OP_READY:
      els_convex_ext_r.op_state = ELS_CONVEX_EXT_OP_MOVEZ0;
      els_stepper_sync();
      break;
...
static void els_convex_ext_r_set_feed(void) {
  ...
  switch(els_keypad_read()) {
    ...
    case ELS_KEY_EXIT:
      ...
      break;
    case ELS_KEY_SET_FEED:
      els_convex_ext_r.state = ELS_CONVEX_EXT_SET_DOC;
      els_convex_ext_r_display_setting();
      break;
    case ELS_KEY_REV_FEED:
      // TODO
    ...
    default:
      ...
      if (els_convex_ext_r.encoder_pos != encoder_curr) {
        ...
        if (els_convex_ext_r.feed_um + delta <= ELS_CONVEX_EXT_FEED_MIN)
          ...
          els_convex_ext_r.feed_um = ELS_CONVEX_EXT_FEED_MAX;
        else
          els_convex_ext_r.feed_um += delta;
    ...
    default:
      ...
      if (els_convex_ext_r.encoder_pos != encoder_curr) {
        ...
        if (els_convex_ext_r.depth_of_cut_um + delta <= ELS_CONVEX_EXT_DOC_MIN)
          ...
          els_convex_ext_r.depth_of_cut_um = ELS_CONVEX_EXT_DOC_MAX;
        else
          els_convex_ext_r.depth_of_cut_um += delta;
        els_convex_ext_r.encoder_pos = encoder_curr;
        els_convex_ext_r_display_setting();
      }
      break;
...
void els_convex_ext_r_set_radius(void) {
  int32_t encoder_curr;
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
    case ELS_KEY_EXIT:
    ...
    default:
      ...
      if (els_convex_ext_r.encoder_pos != encoder_curr) {
        ...
        if (els_convex_ext_r.radius + delta <= 0)
          ...
          els_convex_ext_r.radius = ELS_X_MAX_MM;
        else
          els_convex_ext_r.radius += delta;
        els_convex_ext_r.encoder_pos = encoder_curr;
        if (els_convex_ext_r.depth > els_convex_ext_r.radius)
          els_convex_ext_r.depth = els_convex_ext_r.radius;
        if (els_convex_ext_r.length > els_convex_ext_r.radius)
          els_convex_ext_r.length = els_convex_ext_r.radius;

        // re-adjust length.
        els_convex_ext_r_calculate_arc();
        if (els_convex_ext_r.length > fabs(els_convex_ext_r.arc_center_z)) {
          els_convex_ext_r.length = fabs(els_convex_ext_r.arc_center_z);
          els_convex_ext_r_calculate_arc();
        }
...
void els_convex_ext_r_set_depth(void) {
  int32_t encoder_curr;
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
    case ELS_KEY_EXIT:
...
static void els_convex_ext_r_set_zaxes(void) {
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
    case ELS_KEY_EXIT:
      els_convex_ext_r.state = (els_convex_ext_r.state & ELS_CONVEX_EXT_ZJOG) ? ELS_CONVEX_EXT_SET_ZAXES : ELS_CONVEX_EXT_IDLE;
      els_convex_ext_r_display_axes();
      break;
