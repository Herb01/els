...
static struct {
  ...
  // module state
  els_taper_ext_l_state_t state;

  // tracking variables for display refresh.
  els_taper_ext_l_state_t prev_state;
  els_spindle_direction_t prev_dir;

  // operation state
  els_taper_ext_l_op_state_t op_state;
  els_taper_ext_l_op_state_t prev_op_state;
...
// Internal functions
//==============================================================================
static void els_taper_ext_l_run(void);
static void els_taper_ext_l_turn(void);

static void els_taper_ext_l_display_refresh(void);
static void els_taper_ext_l_display_setting(void);
static void els_taper_ext_l_display_axes(void);
static void els_taper_ext_l_display_header(void);
static void els_taper_ext_l_display_diagram(void);
static void els_taper_ext_l_display_encoder_pips(void);

static void els_taper_ext_l_set_feed(void);
static void els_taper_ext_l_set_depth_of_cut(void);
static void els_taper_ext_l_set_length(void);
static void els_taper_ext_l_set_depth(void);

static void els_taper_ext_l_set_zaxes(void);
static void els_taper_ext_l_set_xaxes(void);

static void els_taper_ext_l_keypad_process(void);

// manual jogging
static void els_taper_ext_l_zjog(void);
static void els_taper_ext_l_xjog(void);

//==============================================================================
// API
//==============================================================================
void els_taper_ext_l_setup(void) {
  // NO-OP
}

void els_taper_ext_l_start(void) {
  char text[32];
  ...
  els_taper_ext_l_display_header();

  if (!els_taper_ext_l.show_dro)
    els_taper_ext_l_display_diagram();
  ...
  // reset state
  els_taper_ext_l.prev_state = 0;
  els_taper_ext_l.prev_dir = 0;
  els_taper_ext_l.feed_mm_s = els_taper_ext_l.feed_um / 1000.0;

  els_taper_ext_l.state = ELS_TAPER_EXT_IDLE;
  els_taper_ext_l.op_state = ELS_TAPER_EXT_OP_IDLE;
  els_taper_ext_l.prev_op_state = ELS_TAPER_EXT_OP_NA;

  els_taper_ext_l_display_setting();
  els_taper_ext_l_display_axes();
  els_taper_ext_l_display_refresh();

  els_stepper_start();
}

void els_taper_ext_l_stop(void) {
  els_stepper_stop();
}

bool els_taper_ext_l_busy(void) {
  return els_taper_ext_l.state != ELS_TAPER_EXT_IDLE;
}

void els_taper_ext_l_update(void) {
  static uint64_t last_refreshed_at = 0;

  uint64_t elapsed = els_timer_elapsed_microseconds();

  bool kp_locked = els_keypad_locked();
  if (els_taper_ext_l.locked != kp_locked) {
    els_taper_ext_l.locked = kp_locked;
    els_taper_ext_l_display_header();
  }
  ...
  switch (els_taper_ext_l.state) {
    ...
    case ELS_TAPER_EXT_SET_FEED:
      els_taper_ext_l_set_feed();
      break;
    case ELS_TAPER_EXT_SET_DOC:
      els_taper_ext_l_set_depth_of_cut();
      break;
    case ELS_TAPER_EXT_SET_LEN:
      els_taper_ext_l_set_length();
      break;
    case ELS_TAPER_EXT_SET_DEPTH:
      els_taper_ext_l_set_depth();
      break;
    default:
      if (els_taper_ext_l.state & (ELS_TAPER_EXT_SET_ZAXES | ELS_TAPER_EXT_ZJOG))
        els_taper_ext_l_set_zaxes();
      else if (els_taper_ext_l.state & (ELS_TAPER_EXT_SET_XAXES | ELS_TAPER_EXT_XJOG))
        els_taper_ext_l_set_xaxes();
      break;
  }
  ...
  if (em != els_taper_ext_l.encoder_multiplier) {
    els_taper_ext_l.encoder_multiplier = em;
    els_taper_ext_l_display_encoder_pips();
  }
}
...
static void els_taper_ext_l_display_setting(void) {
  ...
  els_sprint_double2(text, sizeof(text), els_taper_ext_l.feed_um / 1000.0, "Zf");
  if (els_taper_ext_l.state == ELS_TAPER_EXT_SET_FEED)
  ...
  els_sprint_double2(text, sizeof(text), els_taper_ext_l.depth_of_cut_um / 1000.0, "Xs");
  if (els_taper_ext_l.state == ELS_TAPER_EXT_SET_DOC)
  ...
  els_sprint_double3(text, sizeof(text), els_taper_ext_l.length, "L");
  if (els_taper_ext_l.state == ELS_TAPER_EXT_SET_LEN)
  ...
  els_sprint_double3(text, sizeof(text), els_taper_ext_l.depth, "D");
  if (els_taper_ext_l.state == ELS_TAPER_EXT_SET_DEPTH)
  ...
  if (els_taper_ext_l.only_finish_pass) {
...
static void els_taper_ext_l_display_axes(void) {
  ...
  if (els_taper_ext_l.state & ELS_TAPER_EXT_SET_ZAXES)
  ...
  if (els_taper_ext_l.state & ELS_TAPER_EXT_SET_XAXES)
  ...
  if (els_taper_ext_l.show_dro) {
    ...
    els_sprint_double24(text, sizeof(text), atan(els_taper_ext_l.depth / els_taper_ext_l.length) * 180 / M_PI, NULL);
...
static void els_taper_ext_l_display_diagram(void) {
  ...
  tft_filled_triangle(&tft,  156, 220, 254, 220, 156, 240, ILI9481_ORANGE);
  tft_filled_triangle(&tft,  156, 300, 254, 300, 156, 280, ILI9481_ORANGE);
...
static void els_taper_ext_l_display_header(void) {
  tft_rgb_t color = (els_taper_ext_l.locked ? ILI9481_RED : ILI9481_CERULEAN);

  tft_filled_rectangle(&tft, 0, 0, 480, 50, color);
  tft_font_write_bg(&tft, 8, 0, "TAPER EXTERNAL - L", &noto_sans_mono_bold_26, ILI9481_WHITE, color);
  els_taper_ext_l_display_encoder_pips();
}

static void els_taper_ext_l_display_encoder_pips(void) {
  // multiplier is 100, 10 or 1
  size_t pips = els_taper_ext_l.encoder_multiplier;
  ...
  for (size_t n = 0, spacing = 0; n < 3; n++, spacing++)
    tft_filled_rectangle(&tft,
      ...
      15 + n * 10, 10,
      (n < pips ? ILI9481_WHITE : els_taper_ext_l.locked ? ILI9481_LITEGRAY : ILI9481_BGCOLOR1));
}

static void els_taper_ext_l_display_refresh(void) {
  els_spindle_direction_t dir = els_spindle_get_direction();
  if (dir != els_taper_ext_l.prev_dir) {
    els_taper_ext_l.prev_dir = dir;
  ...
  if (els_taper_ext_l.op_state != els_taper_ext_l.prev_op_state) {
    els_taper_ext_l.prev_op_state = els_taper_ext_l.op_state;
    tft_filled_rectangle(&tft, 310, 195, 169, 35, ILI9481_BLACK);
    if (els_taper_ext_l.op_state == ELS_TAPER_EXT_OP_TURNING && els_taper_ext_l.finish_pass_count > 0) {
      tft_font_write_bg(&tft, 310, 190, "FINISH", &noto_sans_mono_bold_26, ILI9481_CERULEAN, ILI9481_BLACK);
    }
    else {
      const char *label = op_labels[els_taper_ext_l.op_state];
...
static void els_taper_ext_l_keypad_process(void) {
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
      if (els_taper_ext_l.state == ELS_TAPER_EXT_IDLE) {
        if (els_taper_ext_l.length > 0 && els_taper_ext_l.depth > 0) {
          els_taper_ext_l.state = ELS_TAPER_EXT_PAUSED;
          els_taper_ext_l.op_state = ELS_TAPER_EXT_OP_READY;
        }
        else {
          els_taper_ext_l.state = ELS_TAPER_EXT_SET_LEN;
          els_taper_ext_l_display_setting();
        }
      }
      break;
    case ELS_KEY_EXIT:
      if (els_taper_ext_l.state & (ELS_TAPER_EXT_PAUSED | ELS_TAPER_EXT_ACTIVE)) {
        els_taper_ext_l.state = ELS_TAPER_EXT_IDLE;
        els_taper_ext_l.op_state = ELS_TAPER_EXT_OP_IDLE;
      }
      break;
    case ELS_KEY_SET_FEED:
      if (els_taper_ext_l.state & (ELS_TAPER_EXT_IDLE | ELS_TAPER_EXT_PAUSED)) {
        els_taper_ext_l.state = ELS_TAPER_EXT_SET_FEED;
        els_taper_ext_l.encoder_pos = 0;
        els_taper_ext_l_display_setting();
        els_encoder_reset();
      }
      break;
    case ELS_KEY_FUN_F1:
      ...
      break;
    case ELS_KEY_FUN_F2:
      els_taper_ext_l.show_dro = !els_taper_ext_l.show_dro;
    ...
    case ELS_KEY_REV_FEED:
      if (els_taper_ext_l.state & (ELS_TAPER_EXT_IDLE | ELS_TAPER_EXT_PAUSED)) {
        els_taper_ext_l.only_finish_pass = !els_taper_ext_l.only_finish_pass;
        els_taper_ext_l_display_setting();
      }
      break;
    case ELS_KEY_SET_ZX:
      if (els_taper_ext_l.state & (ELS_TAPER_EXT_IDLE | ELS_TAPER_EXT_PAUSED)) {
        els_taper_ext_l.state = ELS_TAPER_EXT_SET_ZAXES;
        els_taper_ext_l.encoder_pos = 0;
        els_taper_ext_l_display_axes();
        els_encoder_reset();
      }
...
static void els_taper_ext_l_run(void) {
  switch (els_spindle_get_direction()) {
    case ELS_S_DIRECTION_CW:
    case ELS_S_DIRECTION_CCW:
      if (els_taper_ext_l.state == ELS_TAPER_EXT_PAUSED && els_spindle_get_counter() == 0)
        els_taper_ext_l.state = ELS_TAPER_EXT_ACTIVE;
      break;
    default:
      if (els_taper_ext_l.state == ELS_TAPER_EXT_ACTIVE)
        els_taper_ext_l.state = ELS_TAPER_EXT_PAUSED;
      break;
  }

  if (els_taper_ext_l.state == ELS_TAPER_EXT_ACTIVE)
    els_taper_ext_l_turn();
}

static void els_taper_ext_l_turn(void) {
  double xd, remaining;

  switch (els_taper_ext_l.op_state) {
    ...
    case ELS_TAPER_EXT_OP_MOVEZL:
      ...
      if (fabs(els_stepper->zpos) > PRECISION)
        els_stepper_move_z(-els_taper_ext_l.length - els_stepper->zpos, els_config->z_retract_jog_mm_s);
      else
    ...
    case ELS_TAPER_EXT_OP_START:
      ...
      els_taper_ext_l.slope = (els_taper_ext_l.length / els_taper_ext_l.depth);
      els_taper_ext_l.op_state = ELS_TAPER_EXT_OP_FEED;
      els_taper_ext_l.finish_pass_count = 0;

      if (els_taper_ext_l.only_finish_pass)
        els_stepper_move_x(-els_taper_ext_l.depth - els_stepper->xpos + 0.1, els_config->x_retract_jog_mm_s);
      break;
    case ELS_TAPER_EXT_OP_FEED:
      if (els_stepper->xbusy)
        break;

      els_taper_ext_l.xcurr = els_stepper->xpos;
      remaining = fabs(els_taper_ext_l.xcurr + els_taper_ext_l.depth);
      if (remaining <= PRECISION) {
        ...
        els_taper_ext_l.op_state = ELS_TAPER_EXT_OP_DONE;
      }
      else {
        if (remaining >= (2 * els_taper_ext_l.depth_of_cut_um) / 1000.0)
          xd = els_taper_ext_l.depth_of_cut_um / 1000.0;
        else if (remaining > (els_taper_ext_l.depth_of_cut_um / 1000.0))
          xd = remaining / 2.0;
        else {
          xd = remaining;
          els_taper_ext_l.finish_pass_count++;
          els_delay_reset();
        }

        els_stepper_move_x(-xd, els_config->x_retract_jog_mm_s);
        els_taper_ext_l.op_state = ELS_TAPER_EXT_OP_PLAN;
      }
      break;
    case ELS_TAPER_EXT_OP_PLAN:
      ...
      els_taper_ext_l.xcurr = els_stepper->xpos;
      els_taper_ext_l.op_state = ELS_TAPER_EXT_OP_TURNING;
      els_stepper_move_xz(
        0 - els_stepper->xpos,
        -els_stepper->xpos * els_taper_ext_l.slope,
        (els_taper_ext_l.finish_pass_count > 0 ? els_taper_ext_l.feed_mm_s / 4 : els_taper_ext_l.feed_mm_s)
      );
      break;
    case ELS_TAPER_EXT_OP_TURNING:
      ...
      if (fabs(els_taper_ext_l.xcurr + els_taper_ext_l.depth) > PRECISION) {
        els_stepper_move_z(-els_taper_ext_l.length - els_stepper->zpos, els_config->z_retract_jog_mm_s);
        els_taper_ext_l.op_state = ELS_TAPER_EXT_OP_RESETZ;
      }
      else {
        els_stepper_move_z(0 - els_stepper->zpos, els_config->z_jog_mm_s);
        els_taper_ext_l.op_state = ELS_TAPER_EXT_OP_DONE;
    ...
    case ELS_TAPER_EXT_OP_RESETZ:
      ...
      els_stepper_move_x(els_taper_ext_l.xcurr - els_stepper->xpos, els_config->x_retract_jog_mm_s);
      els_taper_ext_l.op_state = ELS_TAPER_EXT_OP_RESETX;
      break;
...
static void els_taper_ext_l_set_feed(void) {
  ...
  switch(els_keypad_read()) {
    ...
    case ELS_KEY_EXIT:
      ...
      break;
    case ELS_KEY_SET_FEED:
      els_taper_ext_l.state = ELS_TAPER_EXT_SET_DOC;
      els_taper_ext_l_display_setting();
      break;
    case ELS_KEY_REV_FEED:
      // TODO
    ...
    default:
      ...
      if (els_taper_ext_l.encoder_pos != encoder_curr) {
        int32_t delta = (encoder_curr - els_taper_ext_l.encoder_pos) * 10 * els_taper_ext_l.encoder_multiplier;
        if (els_taper_ext_l.feed_um + delta <= ELS_TAPER_EXT_FEED_MIN)
          els_taper_ext_l.feed_um = ELS_TAPER_EXT_FEED_MIN;
        else if (els_taper_ext_l.feed_um + delta >= ELS_TAPER_EXT_FEED_MAX)
          els_taper_ext_l.feed_um = ELS_TAPER_EXT_FEED_MAX;
        else
          els_taper_ext_l.feed_um += delta;
        els_taper_ext_l.encoder_pos = encoder_curr;
        els_taper_ext_l_display_setting();
        els_taper_ext_l.feed_mm_s = els_taper_ext_l.feed_um / 1000.0;
      }
      break;
...
static void els_taper_ext_l_set_depth_of_cut(void) {
  ...
  switch(els_keypad_read()) {
    ...
    default:
      ...
      if (els_taper_ext_l.encoder_pos != encoder_curr) {
        int32_t delta = (encoder_curr - els_taper_ext_l.encoder_pos) * 10 * els_taper_ext_l.encoder_multiplier;
        if (els_taper_ext_l.depth_of_cut_um + delta <= ELS_TAPER_EXT_DOC_MIN)
          els_taper_ext_l.depth_of_cut_um = ELS_TAPER_EXT_DOC_MIN;
        else if (els_taper_ext_l.depth_of_cut_um + delta >= ELS_TAPER_EXT_DOC_MAX)
          els_taper_ext_l.depth_of_cut_um = ELS_TAPER_EXT_DOC_MAX;
        else
          els_taper_ext_l.depth_of_cut_um += delta;
        els_taper_ext_l.encoder_pos = encoder_curr;
        els_taper_ext_l_display_setting();
      }
      break;
...
void els_taper_ext_l_set_length(void) {
  int32_t encoder_curr;
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
    case ELS_KEY_EXIT:
      ...
      break;
    case ELS_KEY_FUN_F1:
      els_taper_ext_l.state = ELS_TAPER_EXT_SET_DEPTH;
      els_taper_ext_l_display_setting();
      break;
    default:
      encoder_curr = els_encoder_read();
      if (els_taper_ext_l.encoder_pos != encoder_curr) {
        double delta = (encoder_curr - els_taper_ext_l.encoder_pos) * 0.01 * els_taper_ext_l.encoder_multiplier;
        if (els_taper_ext_l.length + delta <= 0)
          els_taper_ext_l.length = 0;
        else if (els_taper_ext_l.length + delta >= ELS_Z_MAX_MM)
          els_taper_ext_l.length = ELS_Z_MAX_MM;
        else
          els_taper_ext_l.length += delta;
        els_taper_ext_l.encoder_pos = encoder_curr;
        els_taper_ext_l_display_setting();
      }
      break;
...
void els_taper_ext_l_set_depth(void) {
  int32_t encoder_curr;
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
    case ELS_KEY_EXIT:
    ...
    default:
      ...
      if (els_taper_ext_l.encoder_pos != encoder_curr) {
        double delta = (encoder_curr - els_taper_ext_l.encoder_pos) * 0.01 * els_taper_ext_l.encoder_multiplier;
        if (els_taper_ext_l.depth + delta <= 0)
          els_taper_ext_l.depth = 0;
        else if (els_taper_ext_l.depth + delta >= ELS_X_MAX_MM)
          els_taper_ext_l.depth = ELS_X_MAX_MM;
        else
          els_taper_ext_l.depth += delta;
        els_taper_ext_l.encoder_pos = encoder_curr;
        els_taper_ext_l_display_setting();
      }
      break;
...
static void els_taper_ext_l_set_zaxes(void) {
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
    case ELS_KEY_EXIT:
      els_taper_ext_l.state = (els_taper_ext_l.state & ELS_TAPER_EXT_ZJOG) ? ELS_TAPER_EXT_SET_ZAXES : ELS_TAPER_EXT_IDLE;
      els_taper_ext_l_display_axes();
      break;
    ...
    case ELS_KEY_JOG_ZX_ORI:
      ...
      break;
    case ELS_KEY_SET_ZX:
      els_taper_ext_l.state = ELS_TAPER_EXT_SET_XAXES;
      els_taper_ext_l_display_axes();
      break;
    default:
      els_taper_ext_l_zjog();
      break;
  }
}
...
static void els_taper_ext_l_zjog(void) {
  ...
  encoder_curr = els_encoder_read();
  if (els_taper_ext_l.encoder_pos != encoder_curr) {
    step = els_taper_ext_l.encoder_multiplier == 1 ? 0.005 : 0.01 * els_taper_ext_l.encoder_multiplier;
    delta = (encoder_curr - els_taper_ext_l.encoder_pos) * step;
    els_taper_ext_l.encoder_pos = encoder_curr;
