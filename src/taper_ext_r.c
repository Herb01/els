...
static struct {
  ...
  // module state
  els_taper_ext_r_state_t state;

  // tracking variables for display refresh.
  els_taper_ext_r_state_t prev_state;
  els_spindle_direction_t prev_dir;

  // operation state
  els_taper_ext_r_op_state_t op_state;
  els_taper_ext_r_op_state_t prev_op_state;
...
// Internal functions
//==============================================================================
static void els_taper_ext_r_run(void);
static void els_taper_ext_r_turn(void);

static void els_taper_ext_r_display_refresh(void);
static void els_taper_ext_r_display_setting(void);
static void els_taper_ext_r_display_axes(void);
static void els_taper_ext_r_display_header(void);
static void els_taper_ext_r_display_diagram(void);
static void els_taper_ext_r_display_encoder_pips(void);

static void els_taper_ext_r_set_feed(void);
static void els_taper_ext_r_set_depth_of_cut(void);
static void els_taper_ext_r_set_length(void);
static void els_taper_ext_r_set_depth(void);

static void els_taper_ext_r_set_zaxes(void);
static void els_taper_ext_r_set_xaxes(void);

static void els_taper_ext_r_keypad_process(void);
...
// API
//==============================================================================
void els_taper_ext_r_setup(void) {
  // NO-OP
}

void els_taper_ext_r_start(void) {
  char text[32];
  ...
  els_taper_ext_r_display_header();

  if (!els_taper_ext_r.show_dro)
    els_taper_ext_r_display_diagram();
  ...
  // reset state
  els_taper_ext_r.prev_state = 0;
  els_taper_ext_r.prev_dir = 0;
  els_taper_ext_r.feed_mm_s = els_taper_ext_r.feed_um / 1000.0;

  els_taper_ext_r.state = ELS_TAPER_EXT_IDLE;
  els_taper_ext_r.op_state = ELS_TAPER_EXT_OP_IDLE;
  els_taper_ext_r.prev_op_state = ELS_TAPER_EXT_OP_NA;

  els_taper_ext_r_display_setting();
  els_taper_ext_r_display_axes();
  els_taper_ext_r_display_refresh();

  els_stepper_start();
}

void els_taper_ext_r_stop(void) {
  els_stepper_stop();
}

bool els_taper_ext_r_busy(void) {
  return els_taper_ext_r.state != ELS_TAPER_EXT_IDLE;
}

void els_taper_ext_r_update(void) {
  static uint64_t last_refreshed_at = 0;

  uint64_t elapsed = els_timer_elapsed_microseconds();

  bool kp_locked = els_keypad_locked();
  if (els_taper_ext_r.locked != kp_locked) {
    els_taper_ext_r.locked = kp_locked;
    els_taper_ext_r_display_header();
  }

  if (els_taper_ext_r.state & (ELS_TAPER_EXT_IDLE | ELS_TAPER_EXT_PAUSED | ELS_TAPER_EXT_ACTIVE))
    els_taper_ext_r_keypad_process();

  switch (els_taper_ext_r.state) {
    case ELS_TAPER_EXT_PAUSED:
    case ELS_TAPER_EXT_ACTIVE:
      els_taper_ext_r_run();
      break;
    case ELS_TAPER_EXT_SET_FEED:
      els_taper_ext_r_set_feed();
      break;
    case ELS_TAPER_EXT_SET_DOC:
      els_taper_ext_r_set_depth_of_cut();
      break;
    case ELS_TAPER_EXT_SET_LEN:
      els_taper_ext_r_set_length();
      break;
    case ELS_TAPER_EXT_SET_DEPTH:
      els_taper_ext_r_set_depth();
      break;
    default:
      if (els_taper_ext_r.state & (ELS_TAPER_EXT_SET_ZAXES | ELS_TAPER_EXT_ZJOG))
        els_taper_ext_r_set_zaxes();
      else if (els_taper_ext_r.state & (ELS_TAPER_EXT_SET_XAXES | ELS_TAPER_EXT_XJOG))
        els_taper_ext_r_set_xaxes();
      break;
  }
  ...
  if (em != els_taper_ext_r.encoder_multiplier) {
    els_taper_ext_r.encoder_multiplier = em;
    els_taper_ext_r_display_encoder_pips();
  }
}
...
static void els_taper_ext_r_display_setting(void) {
  ...
  els_sprint_double2(text, sizeof(text), els_taper_ext_r.feed_um / 1000.0, "Zf");
  if (els_taper_ext_r.state == ELS_TAPER_EXT_SET_FEED)
  ...
  els_sprint_double2(text, sizeof(text), els_taper_ext_r.depth_of_cut_um / 1000.0, "Xs");
  if (els_taper_ext_r.state == ELS_TAPER_EXT_SET_DOC)
  ...
  els_sprint_double3(text, sizeof(text), els_taper_ext_r.length, "L");
  if (els_taper_ext_r.state == ELS_TAPER_EXT_SET_LEN)
  ...
  els_sprint_double3(text, sizeof(text), els_taper_ext_r.depth, "D");
  if (els_taper_ext_r.state == ELS_TAPER_EXT_SET_DEPTH)
...
static void els_taper_ext_r_display_axes(void) {
  ...
  if (els_taper_ext_r.state & ELS_TAPER_EXT_SET_ZAXES)
  ...
  if (els_taper_ext_r.state & ELS_TAPER_EXT_SET_XAXES)
...
static void els_taper_ext_r_display_diagram(void) {
  ...
  tft_filled_triangle(&tft,  156, 220, 254, 240, 254, 220, ILI9481_ORANGE);
  tft_filled_triangle(&tft,  156, 300, 254, 280, 254, 300, ILI9481_ORANGE);
...
static void els_taper_ext_r_display_header(void) {
  tft_rgb_t color = (els_taper_ext_r.locked ? ILI9481_RED : ILI9481_CERULEAN);

  tft_filled_rectangle(&tft, 0, 0, 480, 50, color);
  tft_font_write_bg(&tft, 8, 0, "TAPER EXTERNAL - R", &noto_sans_mono_bold_26, ILI9481_WHITE, color);
  els_taper_ext_r_display_encoder_pips();
}

static void els_taper_ext_r_display_encoder_pips(void) {
  // multiplier is 100, 10 or 1
  size_t pips = els_taper_ext_r.encoder_multiplier;
  ...
  for (size_t n = 0, spacing = 0; n < 3; n++, spacing++)
    tft_filled_rectangle(&tft,
      ...
      15 + n * 10, 10,
      (n < pips ? ILI9481_WHITE : els_taper_ext_r.locked ? ILI9481_LITEGRAY : ILI9481_BGCOLOR1));
}

static void els_taper_ext_r_display_refresh(void) {
  els_spindle_direction_t dir = els_spindle_get_direction();
  if (dir != els_taper_ext_r.prev_dir) {
    els_taper_ext_r.prev_dir = dir;
...
static void els_taper_ext_r_keypad_process(void) {
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
      if (els_taper_ext_r.state == ELS_TAPER_EXT_IDLE) {
        if (els_taper_ext_r.length > 0 && els_taper_ext_r.depth > 0) {
          els_taper_ext_r.state = ELS_TAPER_EXT_PAUSED;
          els_taper_ext_r.op_state = ELS_TAPER_EXT_OP_READY;
        }
        else {
          els_taper_ext_r.state = ELS_TAPER_EXT_SET_LEN;
          els_taper_ext_r_display_setting();
        }
      }
      break;
    case ELS_KEY_EXIT:
      if (els_taper_ext_r.state & (ELS_TAPER_EXT_PAUSED | ELS_TAPER_EXT_ACTIVE)) {
        els_taper_ext_r.state = ELS_TAPER_EXT_IDLE;
        els_taper_ext_r.op_state = ELS_TAPER_EXT_OP_IDLE;
      }
      break;
    case ELS_KEY_SET_FEED:
      if (els_taper_ext_r.state & (ELS_TAPER_EXT_IDLE | ELS_TAPER_EXT_PAUSED)) {
        els_taper_ext_r.state = ELS_TAPER_EXT_SET_FEED;
        els_taper_ext_r.encoder_pos = 0;
        els_taper_ext_r_display_setting();
        els_encoder_reset();
      }
      break;
    case ELS_KEY_FUN_F1:
      ...
      break;
    case ELS_KEY_FUN_F2:
      els_taper_ext_r.show_dro = !els_taper_ext_r.show_dro;
...
static void els_taper_ext_r_run(void) {
  switch (els_spindle_get_direction()) {
    case ELS_S_DIRECTION_CW:
    case ELS_S_DIRECTION_CCW:
      if (els_taper_ext_r.state == ELS_TAPER_EXT_PAUSED && els_spindle_get_counter() == 0)
        els_taper_ext_r.state = ELS_TAPER_EXT_ACTIVE;
      break;
    default:
      if (els_taper_ext_r.state == ELS_TAPER_EXT_ACTIVE)
        els_taper_ext_r.state = ELS_TAPER_EXT_PAUSED;
      break;
  }

  if (els_taper_ext_r.state == ELS_TAPER_EXT_ACTIVE)
    els_taper_ext_r_turn();
}

static void els_taper_ext_r_turn(void) {
  double xd, remaining;

  switch (els_taper_ext_r.op_state) {
    ...
    case ELS_TAPER_EXT_OP_FEED:
      ...
      els_taper_ext_r.xcurr = els_stepper->xpos;
      remaining = fabs(els_taper_ext_r.xcurr + els_taper_ext_r.depth);
      if (remaining <= PRECISION) {
        els_stepper_move_x(0 - els_stepper->xpos, els_config->x_retract_jog_mm_s);
        els_taper_ext_r.op_state = ELS_TAPER_EXT_OP_DONE;
      }
      else {
        if (remaining >= (2 * els_taper_ext_r.depth_of_cut_um) / 1000.0)
          xd = els_taper_ext_r.depth_of_cut_um / 1000.0;
        else if (remaining > (els_taper_ext_r.depth_of_cut_um / 1000.0))
          xd = remaining / 2.0;
        else {
          xd = remaining;
        ...
        els_taper_ext_r.op_state = ELS_TAPER_EXT_OP_PLAN;
      }
      break;
    case ELS_TAPER_EXT_OP_PLAN:
      ...
      els_taper_ext_r.xcurr = els_stepper->xpos;
      els_taper_ext_r.op_state = ELS_TAPER_EXT_OP_TURNING;
      els_stepper_move_xz(
        0 - els_stepper->xpos,
    ...
    case ELS_TAPER_EXT_OP_TURNING:
      ...
      els_taper_ext_r.op_state = ELS_TAPER_EXT_OP_RESETZ;
      els_stepper_move_z(0 - els_stepper->zpos, els_config->z_retract_jog_mm_s);
      break;
    case ELS_TAPER_EXT_OP_RESETZ:
      ...
      if (fabs(els_taper_ext_r.xcurr + els_taper_ext_r.depth) > PRECISION) {
        els_stepper_move_x(els_taper_ext_r.xcurr - els_stepper->xpos, els_config->x_retract_jog_mm_s);
        els_taper_ext_r.op_state = ELS_TAPER_EXT_OP_RESETX;
      }
      else {
        els_taper_ext_r.op_state = ELS_TAPER_EXT_OP_DONE;
      }
...
static void els_taper_ext_r_set_feed(void) {
  ...
  switch(els_keypad_read()) {
    ...
    case ELS_KEY_EXIT:
      ...
      break;
    case ELS_KEY_SET_FEED:
      els_taper_ext_r.state = ELS_TAPER_EXT_SET_DOC;
      els_taper_ext_r_display_setting();
      break;
    case ELS_KEY_REV_FEED:
      // TODO
    ...
    default:
      ...
      if (els_taper_ext_r.encoder_pos != encoder_curr) {
        int32_t delta = (encoder_curr - els_taper_ext_r.encoder_pos) * 10 * els_taper_ext_r.encoder_multiplier;
        if (els_taper_ext_r.feed_um + delta <= ELS_TAPER_EXT_FEED_MIN)
          els_taper_ext_r.feed_um = ELS_TAPER_EXT_FEED_MIN;
        else if (els_taper_ext_r.feed_um + delta >= ELS_TAPER_EXT_FEED_MAX)
          els_taper_ext_r.feed_um = ELS_TAPER_EXT_FEED_MAX;
        else
          els_taper_ext_r.feed_um += delta;
...
static void els_taper_ext_r_set_depth_of_cut(void) {
  ...
  switch(els_keypad_read()) {
    ...
    default:
      ...
      if (els_taper_ext_r.encoder_pos != encoder_curr) {
        int32_t delta = (encoder_curr - els_taper_ext_r.encoder_pos) * 10 * els_taper_ext_r.encoder_multiplier;
        if (els_taper_ext_r.depth_of_cut_um + delta <= ELS_TAPER_EXT_DOC_MIN)
          els_taper_ext_r.depth_of_cut_um = ELS_TAPER_EXT_DOC_MIN;
        else if (els_taper_ext_r.depth_of_cut_um + delta >= ELS_TAPER_EXT_DOC_MAX)
          els_taper_ext_r.depth_of_cut_um = ELS_TAPER_EXT_DOC_MAX;
        else
          els_taper_ext_r.depth_of_cut_um += delta;
        els_taper_ext_r.encoder_pos = encoder_curr;
        els_taper_ext_r_display_setting();
      }
      break;
...
void els_taper_ext_r_set_length(void) {
  int32_t encoder_curr;
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
    case ELS_KEY_EXIT:
      ...
      break;
    case ELS_KEY_FUN_F1:
      els_taper_ext_r.state = ELS_TAPER_EXT_SET_DEPTH;
      els_taper_ext_r_display_setting();
      break;
    default:
      encoder_curr = els_encoder_read();
      if (els_taper_ext_r.encoder_pos != encoder_curr) {
        double delta = (encoder_curr - els_taper_ext_r.encoder_pos) * 0.01 * els_taper_ext_r.encoder_multiplier;
        if (els_taper_ext_r.length + delta <= 0)
          els_taper_ext_r.length = 0;
        else if (els_taper_ext_r.length + delta >= ELS_Z_MAX_MM)
          els_taper_ext_r.length = ELS_Z_MAX_MM;
        else
          els_taper_ext_r.length += delta;
        els_taper_ext_r.encoder_pos = encoder_curr;
        els_taper_ext_r_display_setting();
      }
      break;
...
void els_taper_ext_r_set_depth(void) {
  int32_t encoder_curr;
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
    case ELS_KEY_EXIT:
    ...
    default:
      ...
      if (els_taper_ext_r.encoder_pos != encoder_curr) {
        double delta = (encoder_curr - els_taper_ext_r.encoder_pos) * 0.01 * els_taper_ext_r.encoder_multiplier;
        if (els_taper_ext_r.depth + delta <= 0)
          els_taper_ext_r.depth = 0;
        else if (els_taper_ext_r.depth + delta >= ELS_X_MAX_MM)
          els_taper_ext_r.depth = ELS_X_MAX_MM;
        else
          els_taper_ext_r.depth += delta;
        els_taper_ext_r.encoder_pos = encoder_curr;
        els_taper_ext_r_display_setting();
      }
      break;
...
static void els_taper_ext_r_set_zaxes(void) {
  switch(els_keypad_read()) {
    case ELS_KEY_OK:
    case ELS_KEY_EXIT:
      els_taper_ext_r.state = (els_taper_ext_r.state & ELS_TAPER_EXT_ZJOG) ? ELS_TAPER_EXT_SET_ZAXES : ELS_TAPER_EXT_IDLE;
      els_taper_ext_r_display_axes();
      break;
